...
class GithubRoutes {
  ...
  init() {
    ...
    this.app.get(
      '/github/repos/:owner/:repo/pulls/:pull_number/diff',
    ...
    this.app.get(
      '/github/repos/:owner/:repo/pulls/:pull_number/commits',
      async (req: Request, res: Response) => {
        try {
          const owner = req.params.owner as string;
          const repo = req.params.repo as string;
    ...
    this.app.post(
      '/github/repos/:owner/:repo/pulls/:pull_number/review',
      async (req: Request, res: Response) => {
        try {
          const owner = req.params.owner as string;
          const repo = req.params.repo as string;
          ...
          if (!body || !event)
            return res.status(400).json({ error: 'body and event are required' });
          const data = await this.githubServices.submitPRReview({
            owner,
            repo,
            pull_number,
            body,
