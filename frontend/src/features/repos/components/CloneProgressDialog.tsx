        ...
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between pr-8 text-foreground">
            <div className="flex items-center space-x-2">
              ...
              {!isDone && !isError && <Loader2 className="animate-spin w-4 h-4 text-muted-foreground" />}
              ...
              <div key={`${log.id}-${index}`} className="flex flex-col">
                <div className="flex items-start space-x-2">
                  <div className="mt-0.5 shrink-0">
                    {log.actionProgress === 'pending' && (
                      <Loader2 className="w-4 h-4 text-blue-500 animate-spin" />
                    )}
                    {log.actionProgress === 'success' && (
                      <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                    )}
                    {log.actionProgress === 'error' && (
